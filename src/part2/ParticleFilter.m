...
% the position of the fishes and the predator, using Sequential Monte-Carlo 
% filter.
%
%  ParticleFilter(y,ye,param) uses the Sequential Monte-Carlo filter to 
%  compute an approximation of the true positions of the fishes and of the
%  the predator, based on the noisy measurements y (for the fishes) and ye
%  (for the predator).
%
% Inputs:
...
% Creation: 30-Apr-2019. Last update: 17-May-2019.
...
% The only information we have about the initial position of the fishes and
% of the predator is the noisy measurement y(e) at time t=1. Hence, we take
% this knowledge as the initial position. To simulate particles from only
% one measurement, we add some noise to it.
%  For each particle
for i = 1:Np
    % For each fish
    for fish = 1:P
...
for t = 0:N-2
    ...
    % For each particle, we call the function StateUpdate. It gives the
    % next position and orientation for each fish and for the predator for
    % particle i. We stock the result in Xtilde(e), the prediction set for
    % the fishes and the predator, respectively.
    for i = 1:Np
    ...
    % weights: the weights for the fishes.
    ...
    % The estimated resampled position is stored in the posterior set,
    % X(e).
    for i=1:Np
        for fish = 1:P
            % For the x-value of the fish
