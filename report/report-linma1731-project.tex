...
\section{Estimation of the speed}
Recall that the speed of the fished follows a Gamma distribution (\( V_i \sim \Gamma(k,s) \)). Here, we get noisy measurements of the position of a fish through time. Once we have extracted a noisy approximation of the speed from this set of data, we can repeat what we explained earlier in the document (\ref{sec:est_code}). Hence, we still have to compute these approximations. To do so, some basic physical algebra does the work. From \(x(i)\), the noisy measurement of the position at time $t=i$, we can get \(v^i\), the noisy approximation of the speed of the fish at time $t=i$, by
\begin{align}
	\textbf{v}^i & = \frac{\big|\big| \textbf{x}^{i+1} + \textbf{n}_p - \big(\textbf{x}^{i+1} + \textbf{n}_p(i)\big)\big|\big| }{t_s},
	\label{eq:speed}
\end{align}
With $t_s$ being the the sampling period, and $\textbf{n}_p \sim \mathcal{N}(0, \sigma_{obs})$ a white noise. As said before, using the function computed and explained earlier in the document, we can get approximations of the two parameters of the Gamma distribution of the speed, \((\hat{k}, \hat{s})\). We can the that the zero-mean white noise has no effect on our estimators. Indeed, as shown in equation \ref{eq:speed}, the white-noise from time \(i+1\) is "cancelled" with the white-noise from time \(i\). We hence subtract the bias (which follows the same distribution) of two biased measurements of the position, and it is then "cancelled".
\section{Monte-Carlo filter}
 We now would like to track the position of the fishes and the predator, based on noisy measurements generated by \texttt{GenerateObservations}. Here, we cannot assume that the system follows a linear evolution. Hence, using Kalman filter would give bad results. Fortunately, we can use the Monte-Carlo filter. At each step of time, we must generate a large number of samples from the posterior, and use those samples to generate the next state. HERE LINK TO THE BOOK. The Monte-Carlo filter allows us not to compute those samples at each time step. Before going further, let's introduce notations: \(f(x_t|y_1,...,y_t)\) is the posterior density function at time $t$, and \(x_t^i\) a sample from this posterior. We can define the sample set: \(S_t = \{x_t^i | i=1,2,...,Np\}\). \\
 
 Our goal is to use the observations \(S_t\) and the noisy measurement of the position, \(y_{t+1}\) to compute the sample set \(S_{t+1}\). We can then first create the new sample set; and then correct those values thanks to the noisy measurement.
