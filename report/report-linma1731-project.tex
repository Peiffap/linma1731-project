...
It is rather straightforward to obtain a noisy observation of the speed from these measured positions, knowing the sampling period \(\ts\).
...
We begin by defining the formula for the speed more rigorously: from \(\bm{x}_p(i)\), the noisy measurement of the position of fish \(p\) at step $i$, one can get \(\bm{v}_p(i)\), the noisy observation of the speed of that fish at that step, by
\begin{align}
...
We can see that the zero-mean white noise seems to have no effect on our estimators of the speed.
Indeed, as shown in \eqref{eq:speed}, from the white noise at time \(t+1\) is subtracted the white noise at time \(t\). Since both have the same distribution, on average, the noise is then cancelled, and the measurements we get for the speed tend to be unbiased.
...
In this case, one must exclude a linear evolution, hence using a Kalman filter would give bad results; we can however use a sequential Monte Carlo  (SMC) method.
...
The SMC method does not require us to compute these samples at each time step.
The following notations will be used: \(\pdf(\bm{x}_t \mid \bm{y}_1, \ldots, \bm{y}_t)\) is the posterior density function at time $t$, and \(\bm{x}_t^i\) is a sample from this posterior density.
We also define the sample set \(\sset_t = \{\bm{x}_t^i\}\), for \(i = 1,2,\ldots,\np\).
Our goal is to use the observations in \(\sset_t\) and the noisy measurements of the position, \(\bm{y}_{t+1}\) to compute the sample set \(\sset_{t+1}\).
...
\begin{step}[Initialization]
	...
	It is then filled using noisy observations \(\bm{y}_1\), together with an additive zero-mean white noise for each particle.
	This initial guess allows us to start from the only approximation of the position we know (that is, its observed value).
	To compute the initial orientation of the \(p\)th fish, we use again the noisy measurements from \(\bm{y}_2^p\) and \(\bm{y}_1^p\). Throughout the computation, we keep in memory all the sample sets, for each time step, and for each fish and predator. Since we work in a subset of \(\mathbb{R}^2\), each sample \(\bm{x}_{t}^i\) is a 2-dimensional vector, that is: \(\bm{x}_{t}^i = \begin{pmatrix}
	\xi & \upsilon
	\end{pmatrix}^{T}\).
\end{step}

\begin{step}[Prediction]
	To predict the samples at step \(t+1\), we can use the posterior density samples at step \(t\), using the prediction density \(\pdf(\bm{x}_{t+1} \mid \bm{y}_1,\ldots,\bm{y}_t)\).
	We can compute the prediction set \(\tilde{\sset}_{t+1}\) of samples \(\tilde{\bm{x}}_t^i \).
	One can show that these last samples are samples from \(\pdf(\bm{x}_{t+1} \mid \bm{y}_1, \ldots, \bm{y}_t)\). \cite{anuj5}
...
\begin{step}[Correction]
	We use the prediction set computed earlier, \(\tilde{\sset}_{t+1}\), to generate samples from the posterior density function.
	The goal here is to compute weights \( \bm{w}_{t+1}^i \) for each particle of this set.
	...
	Using these sampling weights, particles which are further from the true value will have a lower weight.
	These weights are then normalized, which gives \(\tilde{\bm{w}}_{t+1}^i\).
	One can show that if we resample from the set \(\tilde{\sset}_{t+1}\) with probabilities given by \( \big\{ \tilde{\bm{w}}_{t+1}^1, \ldots, \tilde{\bm{w}}_{t+1}^{\np} \big\} \), then the resulting values are samples from the posterior density.
	...
	Generating \(\np\) resamples, which we call \(\bm{x}_{t+1}^i\), gives the sample set
	\begin{align}
	\sset_{t+1} &= \left\{ \bm{x}_{t+1}^1, \bm{x}_{t+1}^2, \ldots, \bm{x}_{t+1}^{\np} \right\}\,,
	\end{align}
...
In order to quantify this, we use the mean squared error, defined as\footnote{We use \(\hat{x}\) and \(x_\textnormal{est}\) interchangeably in the code and the paper.}
\begin{align}
E_{\textnormal{MSE}} = \frac{1}{N} \frac{1}{P} \sum_{p=1}^P \sum_{i=1}^N \norm{\bm{x}_p(i) - \hat{\bm{x}}_p(i)}_{2}^2\,.
\end{align}
Experiments were conducted for different values of three parameters.
The default values of the all the parameters were \(w = 20\), \(P = 3\), \(N = 100\), \(\np = 150\), \(\ts = 0.1\) and \(\sigma_{\textnormal{obs}} = 0.2\).
These values were found to have a good mix of ease of computation and interesting results, hence why they were chosen.
...
The filter also becomes more confident about the spread of the value, as seen by the increasingly shorter error bars.
This is what one would expect, when thinking about the underlying mechanism: an increase in the number of particles means a higher chance of one of those particles having a high likelihood, hence being a good estimate for the position.
This can also explain the lower standard deviation, as it implies more particles are likely to have a high likelihood, making them likely candidates for resampling.
We can also note that the asymptotic error seems to be approximately \(0.3\).
Another interesting observation is the fact that by using the value found by simply returning the observed positions, that is, the \(\bm{y}\) vector, one gets a better result than with the filter.
When using more particles per fish, the difference becomes negligible however.
...
\begin{figure}[!htbp]
	...
	Values were chosen so as to both illustrate the strong increase in performance as the sampling period goes down, while also showing the apparent randomness once it reaches ``high'' values, on the order of multiple seconds.}
	\label{fig:t_s}
\end{figure}
...
This means that the shape tends to that of a Dirac distribution, i.e. a deterministic impulse, as \(\ts\) goes to zero.
However, one must be careful to use enough particles: if the probable positions are very clustered, there is a non-negligible chance of not having any particles fall in the probable region, leading to bad results.
...
Finally, the last parameter whose influence was examined is the standard deviation of the observed values, \(\sigma_\textnormal{obs}\).
...
\begin{figure}[!htbp]
	...
	Values were chosen so as to visualize the expected low value near \(\sigma_\textnormal{obs} = 0\) and the increased performance compared to simply using observations as estimates when the standard deviation increases.}
	\label{fig:sigma_obs}
\end{figure}
...
However, when the noise's standard deviation is increased, the spread increases and the mean squared error becomes more important.
...
When the variance goes up, the filtered estimate is much better than simply taking the unfiltered observations, as filtering eliminates part of the noise by using the Monte Carlo technique.

\section{Theoretical improvements of the filter}
\label{sec:improvements}
As we observed in the previous section, the filter is by no means perfect yet.
...
\begin{itemize}
	...
	According to \cite{stattech}, a good value of \(c\) in most cases is \(\frac{1}{M}\), where \(M\) is the dimensionality of the observation model (i.e. \(M = 2\), in this case).
	Other solutions include using a low-pass filter, or convoluting with a Gaussian, in order to ``blur'' the sharpness of the likelihood.
	...
	and if one does decide to resample, the preferred method is low variance resampling. \cite{stachniss}
\end{itemize}
...
Section~\ref{sec:mcfilter} explains the theoretical basis for particle filters, and links this to the practical implementation for this particular project.
Using this implementation, we were able to provide a simulation using both Matlab and Julia. The results of this simulation are summarized, interpreted and discussed in Section~\ref{sec:mseanalysis}.
