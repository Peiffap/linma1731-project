...
\begin{step}[Initialization]
	...
	To compute the initial orientation of the fish, we use again the noisy measurements from \(y_2^p\) and \(y_1^p\). Though all the computation, we keep in memory all the sample sets, for each time step, and for each fish and predator. Since we work in a 2-D space area, each sample \(x_{t}^i\) is a 2-D vector, that is: \(x_{t}^i = \begin{bmatrix}
	x & y
	\end{bmatrix}^{T}\).
\end{step}
