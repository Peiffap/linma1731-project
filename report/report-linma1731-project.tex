...
Figure FIGURE shows the matrix norm induced by the vector 2-norm. We see that, the more N increase, the more we get close to 2 (which is the norm for a matrix 2x2 composed of 1). Figure FIGURE shows also the empirical covariance matrices, the inverse of the Fisher information matrix, and the matrix composed of the the ratio between the coefficients of the first two for a value of $N = 3000$. Wee directly see that these coefficients are very close to one, showing that the covariance matrix tends to the Cramér--Rao lower bound, resulting in the proof that our estimators are efficient.
\begin{figure}[H]
	\centering
